...
cat <<SQL | deno run --allow-net=127.0.0.1:5433 --allow-env ../vault/psql.ts > /dev/null
UPDATE proposals SET proposal_json = proposal_json || '{"curator_notes":{"recommended_tier":"archive","replaces_claims":["smoke-existing-claim"],"net_budget_impact":12,"demotion_candidates":["smoke-existing-claim"]}}'::jsonb
WHERE id = 'urn:cl:proposal:$RUN_ID-claptrap';
SQL
...
echo "$LIST2" | grep -q 'curator advisory: recommended tier archive; net budget impact 12 chars' || { echo "FAIL: list curator advisory missing"; echo "$LIST2"; exit 1; }
...
echo "$PREVIEW" | grep -q 'curator advisory: recommended tier archive; net budget impact 12 chars' || { echo "FAIL: show curator advisory missing"; echo "$PREVIEW"; exit 1; }
echo "$PREVIEW" | grep -q 'replaces: smoke-existing-claim' || { echo "FAIL: show curator replaces line missing"; echo "$PREVIEW"; exit 1; }
